...
function docHeader(s, docType, docNo, date, dueDate, toName, toAttn, toAddress, extra="") {
  ...
  <div class="meta-box"><span class="meta-label">${docType} No.</span><div class="meta-value">${docNo||""}</div><div style="margin-top:8px"><span class="meta-label">Date </span><span class="meta-value">${fmtDate(date)}</span></div>${dueDate?`<div><span class="meta-label">Due </span><span class="meta-value">${fmtDate(dueDate)}</span></div>`:""}${extra}</div></div>`;
}
...
// ─── IMPROVEMENT 5: Client Address Book ──────────────────────────────────────
...
const [fMonth,setFMonth]=useState("");;const [doc,setDoc]=useState(null);
  ...
  const convertToInvoice=(q)=>{
    const prefill={doc_no:"",client:q.client,attn:q.attn||"",address:q.address||"",date:today(),due_date:"",status:"Draft",notes:q.notes||"",terms:q.terms||settings.terms_inv||"",ref_quo:q.doc_no,discount:q.discount||0,tax_rate:q.tax_rate||0,items:(q.items||[]).map(i=>({...i,id:uid()}))};
  ...
  const printQ=(q)=>{
    ...
    printDoc(docHeader(settings,"QUOTATION",q.doc_no,q.date,null,q.client,q.attn,q.address,"")+
      ...
      `<div class="footer">Thank you for your consideration · ${settings.company}</div>`,`Quotation ${q.doc_no}`);
  };
      ...
      {loading?<Spinner/>:rows.length===0?<Empty text="No quotations yet"/>:(
        <div style={{overflowX:"auto"}}><table style={css.table}>
          ...
          <tbody>{rows.map(q=><tr key={q.id}>
            ...
            <td style={css.td}>{fmtDate(q.date)}</td><td style={css.td}>{fmtDate(q.valid_until)}</td>
...
function InvoiceModule({settings}) {
  ...
  useEffect(()=>{
    ...
    const handler=()=>{
      ...
      if(raw){try{const data=JSON.parse(raw);
        dbLoad("invoices").then(existing=>{data.doc_no=existing.length===0?NEXT_INV:nextDocNo("INV-",existing);setDoc({discount:0,tax_rate:0,...data});setEditId(null);setForm(true);sessionStorage.removeItem("prefill_invoice");});
  ...
  setDoc({
    ...
    ...r,
    items: Array.isArray(r.items) && r.items.length ? r.items : [newItem()]
  });
  setEditId(r.id);
  setForm(true);
};
  ...
  const toggleStatus=async(inv)=>{const s=inv.status==="Paid"?"Pending":"Paid";await dbUpdate("invoices",inv.id,{status:s});setRows(rows.map(r=>r.id===inv.id?{...r,status:s}:r));};

  const printI=(inv)=>{
    const items=inv.items||[];const {subtotal,discountAmt,taxAmt,total}=calcDoc(items,inv.discount,inv.tax_rate);
    const trs=items.map((i,idx)=>`<tr><td>${idx+1}</td><td>${i.desc}</td><td style="text-align:right">${nf(i.qty)}</td><td>${i.unit}</td><td style="text-align:right">${fmtMY(i.price)}</td><td style="text-align:right;font-weight:600">${fmtMY(nf(i.qty)*nf(i.price))}</td></tr>`).join("");
    printDoc(docHeader(settings,"INVOICE",inv.doc_no,inv.date,inv.due_date||(inv.payment_terms_days&&inv.payment_terms_days!=="Custom"?(()=>{const d=new Date(inv.date);d.setDate(d.getDate()+parseInt(inv.payment_terms_days));return d.toISOString().slice(0,10);})():""),inv.client,inv.attn,inv.address,
      (inv.payment_terms_days?"<div style=\"margin-top:4px\"><span class=\"meta-label\">Payment Terms </span><span class=\"meta-value\">"+inv.payment_terms_days+"</span></div>":"")+(inv.ref_quo?"<div style=\"margin-top:4px\"><span class=\"meta-label\">Ref QUO </span><span class=\"meta-value\">"+inv.ref_quo+"</span></div>":""))+
      `<table><thead><tr><th>#</th><th>Description</th><th style="text-align:right">Qty</th><th>Unit</th><th style="text-align:right">Unit Price</th><th style="text-align:right">Amount</th></tr></thead><tbody>${trs}</tbody></table>`+
      ...
      `<div class="note-box"><strong>Payment Details</strong><br/>${(settings.payment_terms||("Bank: "+(settings.bankName||"")+"\nAcc: "+(settings.bankAcc||""))).replace(/\n/g,"<br/>")}</div>`+
      `<div class="footer">Thank you for your business · ${settings.company}</div>`,`Invoice ${inv.doc_no}`);
  };
...
function CostingModule({settings}) {
  ...
  const printC=(c)=>{
    ...
    printDoc(docHeader(settings,"PROJECT COSTING",c.doc_no,c.date,null,c.client,"","",`<div style="margin-top:8px"><span class="meta-label">Project </span><span class="meta-value">${c.project}</span></div>`)+
      ...
      `<div class="footer">Prepared by ${settings.company}</div>`,`Costing ${c.doc_no}`);
  };
...
function SupplierModule({settings}) {
  ...
  const printP=(p)=>{
    printDoc(docHeader(settings,"PAYMENT VOUCHER",p.doc_no,p.date,p.due_date,p.supplier,"","",`<div style="margin-top:8px"><span class="meta-label">Method </span><span class="meta-value">${p.method}</span></div>`)+
      ...
      `<div class="footer">${settings.company}</div>`,`Payment Voucher ${p.doc_no}`);
  };
  ...
  if(form&&doc) return(<div>
    ...
    <div style={css.card}>
      <div style={css.grid2}>
        ...
        <div><label style={css.label}>Status</label><select style={css.input} value={doc.status} onChange={e=>setDoc(d=>({...d,status:e.target.value}))}>
...
function SalaryModule({settings}) {
  ...
  const printSlip=(r)=>{
    ...
    printDoc(`<div class="header"><div>${logo}</div><div style="text-align:right"><div class="company-name">${settings.company}</div><div style="color:#666;font-size:12px;line-height:1.7">${settings.address}<br/>${settings.phone}</div></div></div>
    ...
    <div class="footer">${settings.company} — Confidential</div>`,`Salary Slip ${r.doc_no}`);
  };
  const sColor={Paid:C.success,Pending:C.warning};
  if(form&&doc) return(<div>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:24}}>
...
// ─── IMPROVEMENT 6: P&L REPORT ────────────────────────────────────────────────
function PLReport() {
  const [data,setData]=useState(null);const [period,setPeriod]=useState("all");
  ...
  const calcInvT=(i)=>{const sub=(i.items||[]).reduce((s,x)=>s+nf(x.qty)*nf(x.price),0);const disc=nf(i.discount);const tax=((sub-disc)*nf(i.tax_rate))/100;return sub-disc+tax;};
...
function SettingsModule({settings,setSettings}) {
  const [local,setLocal]=useState({...settings});const [saving,setSaving]=useState(false);const [saved,setSaved]=useState(false);
...
function Dashboard({settings}) {
  ...
  return(<div>
    <div style={{marginBottom:28}}><div style={css.pageTitle}>Welcome, {(settings.company||"").split(" ")[0]} 👋</div><div style={css.pageSub}>Business overview · live from cloud</div></div>
